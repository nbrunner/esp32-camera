...
static int reset(sensor_t *sensor)
{
    ...
    write_regs(sensor->slv_addr, gc032a_init_qvga);
    if (ret == 0) {
        ESP_LOGD(TAG, "Camera defaults loaded");
        vTaskDelay(100 / portTICK_PERIOD_MS);
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        set_reg_bits(sensor->slv_addr, 0x46, 0, 0x01, 1);  //vsync polarity
        set_reg_bits(sensor->slv_addr, 0x46, 1, 0x01, 1);  //hsync polarity
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    /*SPI*/
    write_reg(sensor->slv_addr, 0xfe, 0x03);
    write_reg(sensor->slv_addr, P3_IMAGE_WIDTH_LOW, L8(w));
    write_reg(sensor->slv_addr, P3_IMAGE_WIDTH_HIGH, H8(w));
    write_reg(sensor->slv_addr, P3_IMAGE_HEIGHT_LOW, L8(h));
    write_reg(sensor->slv_addr, P3_IMAGE_HEIGHT_HIGH, H8(h));
